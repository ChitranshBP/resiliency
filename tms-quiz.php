    ...
    <section class="py-16 bg-neutral relative overflow-hidden">
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="bg-white rounded-[2rem] shadow-[0_25px_50px_-10px_rgba(11,32,53,0.15)] border border-gray-100 overflow-hidden reveal">
                ...
                <!-- Hidden iframe target so the form posts without navigating away -->
                <iframe id="formester-target" name="formester-target" style="display:none;width:0;height:0;border:0;" aria-hidden="true" tabindex="-1"></iframe>

                <!-- Quiz Form -->
                <form id="tms-quiz-form" accept-charset="UTF-8" action="https://app.formester.com/forms/ZyOFGCPD2/submissions" method="POST" target="formester-target" class="p-6 sm:p-10">
                ...
                <div id="quiz-success" class="hidden p-6 sm:p-10 text-center scroll-mt-32">
...
(function() {
    ...
    function showStep(n) {
        ...
        // Scroll the quiz card into view on step change (offset to clear fixed navbar)
        const card = form.closest('.bg-white');
        const navOffset = 120;
        const targetY = card.getBoundingClientRect().top + window.pageYOffset - navOffset;
    ...
    const iframe = document.getElementById('formester-target');
    ...
    form.addEventListener('submit', (e) => {
        ...
        // Form submits natively to the hidden iframe (target="formester-target").
        // When the iframe finishes loading the Formester response, show success.
        iframe.addEventListener('load', () => {
            showSuccess();
        }, { once: true });

        // Safety net: if the iframe never loads (network error), restore the button after 15s.
        setTimeout(() => {
            ...
            if (typeof lucide !== 'undefined') lucide.createIcons();
            alert('Submission is taking longer than expected. Please try again or call (949) 996-5355.');
        }, 15000);
    });
    ...
    function showSuccess() {
        ...
        if (typeof lucide !== 'undefined') lucide.createIcons();
        // Scroll with offset so success header sits below the fixed navbar
        const navOffset = 120;
        const targetY = successScreen.getBoundingClientRect().top + window.pageYOffset - navOffset;
        window.scrollTo({ top: targetY, behavior: 'smooth' });
    }
})();
</script>
