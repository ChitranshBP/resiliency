<?php /* Template Name: Condition - PTSD */ ?>
<?php
$page_title       = 'PTSD Treatment Newport Beach &amp; Orange County | Deep TMS &amp; Psychiatric Care | Resiliency Mind + Body Medicine';
$page_description = 'Compassionate PTSD treatment in Newport Beach &amp; Orange County, CA. FDA-approved Deep TMS, SPRAVATO &amp; medication management to help you find peace and healing. Call (949) 996-5355.';
$page_keywords    = 'PTSD treatment Newport Beach, PTSD psychiatrist Orange County, TMS for PTSD Newport Beach CA, trauma treatment Orange County, PTSD therapy Newport Beach, post-traumatic stress disorder treatment Orange County CA, PTSD doctor Newport Beach';
$page_canonical   = 'https://resiliencymbmedicine.com/ptsd.php';
$page_schema = <<<'SCHEMA'
<script type="application/ld+json">
{"@context":"https://schema.org","@type":"MedicalCondition","name":"Post-Traumatic Stress Disorder","alternateName":["PTSD","Trauma","Post-Traumatic Stress"],"description":"Compassionate PTSD treatment using Deep TMS and SPRAVATO in Newport Beach and San Juan Capistrano, CA. FDA-approved treatments for trauma survivors.","url":"https://resiliencymbmedicine.com/ptsd.php","relevantSpecialty":"Psychiatry","signOrSymptom":[{"@type":"MedicalSymptom","name":"Flashbacks"},{"@type":"MedicalSymptom","name":"Hypervigilance"},{"@type":"MedicalSymptom","name":"Avoidance"},{"@type":"MedicalSymptom","name":"Nightmares"}],"possibleTreatment":[{"@type":"MedicalTherapy","name":"Deep TMS Therapy","url":"https://resiliencymbmedicine.com/tms-therapy.php"},{"@type":"MedicalTherapy","name":"SPRAVATO Treatment","url":"https://resiliencymbmedicine.com/spravato-therapy.php"},{"@type":"MedicalTherapy","name":"Medication Management","url":"https://resiliencymbmedicine.com/medication-management.php"}]}
</script>
<script type="application/ld+json">
{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"What is Deep TMS for PTSD and how does it work?","acceptedAnswer":{"@type":"Answer","text":"Deep TMS targets the prefrontal cortex and limbic system regions involved in fear processing and emotional regulation, helping reduce PTSD symptoms including hypervigilance, flashbacks, and avoidance."}},{"@type":"Question","name":"How will I know if Deep TMS for PTSD is working?","acceptedAnswer":{"@type":"Answer","text":"We monitor PTSD symptom severity using validated scales throughout treatment. Most patients notice meaningful improvement in hyperarousal and mood within the first 2-3 weeks."}},{"@type":"Question","name":"What are the side effects of Deep TMS for PTSD?","acceptedAnswer":{"@type":"Answer","text":"Side effects are mild and include scalp discomfort or headache during early sessions. Our Newport Beach team monitors closely and adjusts treatment parameters for comfort."}},{"@type":"Question","name":"How long is a course of Deep TMS for PTSD?","acceptedAnswer":{"@type":"Answer","text":"A standard course is 20-30 sessions over 4-6 weeks, with daily 20-minute sessions Monday through Friday. Maintenance sessions may be recommended afterward."}},{"@type":"Question","name":"Do you treat veterans with PTSD?","acceptedAnswer":{"@type":"Answer","text":"Yes. We have experience treating veterans and first responders with combat and duty-related PTSD. We provide compassionate, specialized care for those who have served."}},{"@type":"Question","name":"Can PTSD be treated alongside depression or anxiety?","acceptedAnswer":{"@type":"Answer","text":"Yes. PTSD commonly co-occurs with depression and anxiety. Our multi-modal approach addresses all co-occurring conditions simultaneously for comprehensive symptom relief."}}]}
</script>
SCHEMA;
?>
<?php get_header(); ?>

<main>
    <!-- Hero Section -->
    <section class="relative min-h-[500px] md:h-[60vh] flex items-center justify-center overflow-hidden pt-28 pb-12 md:pt-24 md:pb-0">
        <div class="absolute inset-0 z-0 bg-[#0B2035]">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/hero_bg.png" alt="PTSD Treatment Peace" class="w-full h-full object-cover">
            <div class="absolute inset-0 bg-[#0B2035]/60 md:bg-gradient-to-r md:from-[#0B2035]/75 md:via-[#0B2035]/50 md:to-[#0B2035]/20"></div>
        </div>

        <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
            <div class="max-w-3xl reveal text-left">
                <div class="flex items-center gap-3 mb-4 sm:mb-6">
                    <span class="h-[2px] w-12 bg-[#23bfe2]"></span>
                    <span class="text-[#23bfe2] text-xs sm:text-sm font-bold tracking-widest uppercase">Mental Health Treatment</span>
                </div>
                <h1 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white tracking-tight leading-tight mb-4 sm:mb-6">
                    Deep TMS for PTSD <span class="text-[#23bfe2] font-light">Treatment</span>
                    <span class="block text-base sm:text-lg md:text-xl font-normal text-white/70 mt-2 tracking-normal">in Newport Beach &amp; San Juan Capistrano, CA</span>
                </h1>
                <p class="text-base sm:text-lg md:text-xl text-gray-200 mb-6 sm:mb-10 leading-relaxed font-light max-w-2xl">
                    Find peace and healing with FDA-approved Deep TMS therapy, medication management, and integrative care at Resiliency Mind+Body Medicine.
                </p>
                <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
                    <a href="/contact/" class="bg-[#23bfe2] text-[#0B2035] font-bold px-8 py-4 rounded-full text-center transition-all hover:bg-white hover:shadow-[0_10px_30px_rgba(35,191,226,0.3)] shadow-md hover:-translate-y-1 transform flex items-center justify-center gap-2">
                        Schedule Your Consultation <i data-lucide="arrow-right" class="w-5 h-5"></i>
                    </a>
                    <a href="tel:9499965355" class="bg-transparent border border-white/30 text-white font-medium px-8 py-4 rounded-full text-center transition-all hover:bg-white/10 flex items-center justify-center gap-2">
                        <i data-lucide="phone" class="w-5 h-5"></i> (949) 996-5355
                    </a>
                </div>
            </div>
        </div>
    </section>

    <!-- Understanding the Condition -->
    <section class="py-16 relative overflow-hidden bg-white">
        <div class="absolute top-0 right-0 w-1/3 h-full bg-[#23bfe2]/5 -skew-x-12 transform translate-x-32 hidden lg:block pointer-events-none"></div>

        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="flex flex-col lg:flex-row items-center gap-16 lg:gap-20">
                <div class="w-full lg:w-1/2 text-[#0B2035] reveal">
                    <div class="flex items-center gap-3 mb-4">
                        <span class="h-[2px] w-12 bg-[#23bfe2]"></span>
                        <h2 class="text-sm font-bold tracking-widest text-[#23bfe2] uppercase">Understanding PTSD</h2>
                    </div>
                    <h3 class="text-3xl md:text-4xl lg:text-5xl font-bold mb-6">Understanding PTSD</h3>
                    <p class="text-gray-600 text-lg mb-6 leading-relaxed font-light">
                        PTSD is a mental health condition triggered by experiencing or witnessing a traumatic event. It can lead to severe anxiety, flashbacks, nightmares, and uncontrollable thoughts about the event. PTSD can affect anyone who has gone through trauma, and its symptoms can significantly impact daily life.
                    </p>

                    <div class="space-y-3 mb-6">
                        <div class="flex items-start gap-3">
                            <div class="bg-[#23bfe2]/20 p-1.5 rounded-lg text-[#23bfe2] mt-0.5 flex-shrink-0">
                                <i data-lucide="eye" class="w-4 h-4"></i>
                            </div>
                            <p class="text-gray-700 text-sm"><span class="font-bold text-[#0B2035]">Intrusive Memories</span> — Recurrent, unwanted distressing memories, flashbacks, or nightmares</p>
                        </div>
                        <div class="flex items-start gap-3">
                            <div class="bg-[#23bfe2]/20 p-1.5 rounded-lg text-[#23bfe2] mt-0.5 flex-shrink-0">
                                <i data-lucide="shield-off" class="w-4 h-4"></i>
                            </div>
                            <p class="text-gray-700 text-sm"><span class="font-bold text-[#0B2035]">Avoidance</span> — Avoiding places, activities, or people that remind you of the traumatic event</p>
                        </div>
                        <div class="flex items-start gap-3">
                            <div class="bg-[#23bfe2]/20 p-1.5 rounded-lg text-[#23bfe2] mt-0.5 flex-shrink-0">
                                <i data-lucide="cloud" class="w-4 h-4"></i>
                            </div>
                            <p class="text-gray-700 text-sm"><span class="font-bold text-[#0B2035]">Negative Changes in Thinking and Mood</span> — Negative thoughts, hopelessness, memory problems, difficulty maintaining relationships</p>
                        </div>
                        <div class="flex items-start gap-3">
                            <div class="bg-[#23bfe2]/20 p-1.5 rounded-lg text-[#23bfe2] mt-0.5 flex-shrink-0">
                                <i data-lucide="alert-triangle" class="w-4 h-4"></i>
                            </div>
                            <p class="text-gray-700 text-sm"><span class="font-bold text-[#0B2035]">Changes in Physical and Emotional Reactions</span> — Being easily startled, always on guard, trouble sleeping, irritability</p>
                        </div>
                    </div>
                </div>

                <div class="w-full lg:w-1/2 relative reveal reveal-delay-1 flex justify-center">
                    <div class="rounded-[2.5rem] overflow-hidden shadow-2xl border border-gray-100 ring-1 ring-[#0B2035]/5 w-[320px] h-[568px]">
                        <iframe
                            src="https://www.youtube.com/embed/I70XPpkKwJA"
                            title="Deep TMS for PTSD Treatment at Resiliency Mind+Body Medicine"
                            frameborder="0"
                            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                            allowfullscreen
                            class="w-full h-full"
                            loading="lazy"
                        ></iframe>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- Can Deep TMS Help With PTSD? -->
    <section class="py-16 bg-neutral relative overflow-hidden">
        <div class="absolute -top-32 -left-32 w-96 h-96 bg-[#23bfe2]/10 rounded-full blur-[100px] pointer-events-none"></div>

        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="flex flex-col lg:flex-row items-center gap-16 lg:gap-20">
                <!-- Video (YT Short - 9:16 vertical) -->
                <div class="w-full lg:w-1/2 relative reveal flex justify-center">
                    <div class="rounded-[2.5rem] overflow-hidden shadow-2xl border border-gray-100 ring-1 ring-[#0B2035]/5 w-[320px] h-[568px]">
                        <iframe
                            src="https://www.youtube.com/embed/ryMOtYt7SAw"
                            title="Deep TMS for PTSD at Resiliency Mind+Body Medicine"
                            frameborder="0"
                            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                            allowfullscreen
                            class="w-full h-full"
                            loading="lazy"
                        ></iframe>
                    </div>
                </div>

                <!-- Content -->
                <div class="w-full lg:w-1/2 text-[#0B2035] reveal reveal-delay-1">
                    <div class="flex items-center gap-3 mb-4">
                        <span class="h-[2px] w-12 bg-[#23bfe2]"></span>
                        <h2 class="text-sm font-bold tracking-widest text-[#23bfe2] uppercase">Deep TMS & PTSD</h2>
                    </div>
                    <h3 class="text-3xl md:text-4xl lg:text-5xl font-bold mb-6">Can Deep TMS Help With PTSD?</h3>
                    <p class="text-gray-600 text-lg mb-6 leading-relaxed font-light">
                        Transcranial Magnetic Stimulation (Deep TMS) is a non-invasive neuromodulation treatment that uses magnetic pulses to influence brain circuits involved in mood regulation, fear processing, and emotional control. While Deep TMS is FDA-cleared for major depressive disorder, clinical evidence supports its use in carefully selected patients with PTSD, particularly when symptoms persist despite psychotherapy and medication trials.
                    </p>
                    <p class="text-gray-600 text-lg leading-relaxed font-light">
                        At Resiliency, Deep TMS for PTSD is offered under close psychiatric supervision and integrated into a personalized care plan to support safety and effectiveness.
                    </p>
                </div>
            </div>
        </div>
    </section>

    <!-- How Deep TMS Works + Benefits -->
    <section class="py-16 bg-[#0B2035] relative overflow-hidden text-white border-t border-white/10">
        <div class="absolute inset-0 z-0 bg-gradient-to-b from-[#0B2035] via-[#0B2035]/95 to-[#0B2035]"></div>
        <div class="absolute -top-40 -left-40 w-96 h-96 bg-[#23bfe2]/10 rounded-full blur-[100px] pointer-events-none"></div>

        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="text-center max-w-3xl mx-auto mb-16 reveal">
                <div class="flex items-center justify-center gap-3 mb-4">
                    <span class="h-[2px] w-8 bg-[#23bfe2]"></span>
                    <h2 class="text-sm font-bold tracking-widest text-[#23bfe2] uppercase">The Science</h2>
                    <span class="h-[2px] w-8 bg-[#23bfe2]"></span>
                </div>
                <h3 class="text-4xl md:text-5xl font-bold mb-6">How Deep TMS Works for PTSD</h3>
                <p class="text-gray-300 text-lg font-light leading-relaxed">
                    Transcranial magnetic stimulation (Deep TMS) targets specific brain areas involved in mood regulation and stress response. By delivering magnetic pulses to these regions, Deep TMS can help modulate brain activity, reducing symptoms of PTSD and improving overall mental health.
                </p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div class="bg-white/5 border border-white/10 rounded-[2rem] p-10 hover:-translate-y-2 transition-all duration-300 reveal group text-center">
                    <div class="w-16 h-16 bg-[#23bfe2]/20 rounded-2xl flex items-center justify-center text-[#23bfe2] mb-6 mx-auto group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                        <i data-lucide="trending-down" class="w-8 h-8"></i>
                    </div>
                    <h4 class="text-xl font-bold mb-3">Reduction in PTSD Symptoms</h4>
                    <p class="text-gray-400 font-light leading-relaxed">Many patients experience a significant decrease in symptoms such as flashbacks, anxiety, and hyperarousal.</p>
                </div>

                <div class="bg-white/5 border border-white/10 rounded-[2rem] p-10 hover:-translate-y-2 transition-all duration-300 reveal reveal-delay-1 group text-center">
                    <div class="w-16 h-16 bg-[#23bfe2]/20 rounded-2xl flex items-center justify-center text-[#23bfe2] mb-6 mx-auto group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                        <i data-lucide="shield-check" class="w-8 h-8"></i>
                    </div>
                    <h4 class="text-xl font-bold mb-3">Non-Invasive and Drug-Free</h4>
                    <p class="text-gray-400 font-light leading-relaxed">Deep TMS does not require medication, avoiding potential side effects of pharmaceuticals.</p>
                </div>

                <div class="bg-white/5 border border-white/10 rounded-[2rem] p-10 hover:-translate-y-2 transition-all duration-300 reveal reveal-delay-2 group text-center">
                    <div class="w-16 h-16 bg-[#23bfe2]/20 rounded-2xl flex items-center justify-center text-[#23bfe2] mb-6 mx-auto group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                        <i data-lucide="sparkles" class="w-8 h-8"></i>
                    </div>
                    <h4 class="text-xl font-bold mb-3">Improved Quality of Life</h4>
                    <p class="text-gray-400 font-light leading-relaxed">Deep TMS can help restore normal functioning, allowing patients to engage in everyday activities without the constant burden of PTSD symptoms.</p>
                </div>
            </div>
        </div>
    </section>

    <!-- Deep TMS Treatment Process -->
    <section class="py-16 bg-white relative overflow-hidden">
        <div class="absolute -bottom-32 -left-32 w-96 h-96 bg-[#23bfe2]/10 rounded-full blur-[100px] pointer-events-none"></div>

        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="text-center max-w-3xl mx-auto mb-16 reveal">
                <div class="flex items-center justify-center gap-3 mb-4">
                    <span class="h-[2px] w-8 bg-[#23bfe2]"></span>
                    <h2 class="text-sm font-bold tracking-widest text-[#23bfe2] uppercase">Your Journey</h2>
                    <span class="h-[2px] w-8 bg-[#23bfe2]"></span>
                </div>
                <h3 class="text-4xl md:text-5xl font-bold text-[#0B2035] mb-6">The Deep TMS Treatment Process</h3>
            </div>

            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="relative reveal group">
                    <div class="bg-neutral rounded-[2rem] p-10 border border-gray-100 hover:shadow-xl transition-all duration-300 h-full">
                        <div class="text-6xl font-bold text-[#23bfe2]/20 mb-4 group-hover:text-[#23bfe2]/40 transition-colors">01</div>
                        <h4 class="text-xl font-bold text-[#0B2035] mb-4">Comprehensive Evaluation</h4>
                        <p class="text-gray-500 font-light leading-relaxed">
                            Our team assesses your medical history, current symptoms, and treatment goals to determine if Deep TMS is the right choice for you. This personalized consultation ensures a tailored approach.
                        </p>
                    </div>
                    <div class="hidden lg:block absolute top-1/2 -right-4 text-[#23bfe2]/30">
                        <i data-lucide="chevron-right" class="w-8 h-8"></i>
                    </div>
                </div>

                <div class="relative reveal reveal-delay-1 group">
                    <div class="bg-neutral rounded-[2rem] p-10 border border-gray-100 hover:shadow-xl transition-all duration-300 h-full">
                        <div class="text-6xl font-bold text-[#23bfe2]/20 mb-4 group-hover:text-[#23bfe2]/40 transition-colors">02</div>
                        <h4 class="text-xl font-bold text-[#0B2035] mb-4">Personalized Deep TMS Sessions</h4>
                        <p class="text-gray-500 font-light leading-relaxed">
                            Deep TMS sessions typically last about 10 minutes and are conducted five days a week for several weeks. You sit comfortably while magnetic pulses are delivered—no anesthesia needed, resume activities immediately.
                        </p>
                    </div>
                    <div class="hidden lg:block absolute top-1/2 -right-4 text-[#23bfe2]/30">
                        <i data-lucide="chevron-right" class="w-8 h-8"></i>
                    </div>
                </div>

                <div class="relative reveal reveal-delay-2 group">
                    <div class="bg-neutral rounded-[2rem] p-10 border border-gray-100 hover:shadow-xl transition-all duration-300 h-full">
                        <div class="text-6xl font-bold text-[#23bfe2]/20 mb-4 group-hover:text-[#23bfe2]/40 transition-colors">03</div>
                        <h4 class="text-xl font-bold text-[#0B2035] mb-4">Continuous Support</h4>
                        <p class="text-gray-500 font-light leading-relaxed">
                            Our team provides continuous support, closely monitoring your progress and making adjustments to ensure optimal results. Our goal is lasting relief from PTSD symptoms and regaining your sense of peace.
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- Why Choose Resiliency -->
    <section class="py-16 bg-neutral relative overflow-hidden">
        <div class="absolute -top-32 -right-32 w-96 h-96 bg-[#23bfe2]/10 rounded-full blur-[100px] pointer-events-none"></div>

        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center max-w-3xl mx-auto mb-16 reveal">
                <h2 class="text-sm font-bold tracking-widest text-[#23bfe2] uppercase mb-4">The Resiliency Difference</h2>
                <h3 class="text-4xl md:text-5xl font-bold text-[#0B2035] mb-6">Why Choose Resiliency</h3>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                <div class="bg-white rounded-[2rem] p-8 shadow-soft border border-gray-100 hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 reveal group flex flex-col items-center text-center">
                    <div class="w-14 h-14 bg-[#23bfe2]/10 rounded-2xl flex items-center justify-center text-[#23bfe2] mb-6 group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                        <i data-lucide="user-check" class="w-7 h-7"></i>
                    </div>
                    <h4 class="text-xl font-bold text-[#0B2035] mb-3">Board-Certified Expertise</h4>
                    <p class="text-gray-500 font-light leading-relaxed">Led by Dr. LaFrenierre, double board-certified in Psychiatry and Addiction Medicine, with a multidisciplinary team.</p>
                </div>

                <div class="bg-white rounded-[2rem] p-8 shadow-soft border border-gray-100 hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 reveal reveal-delay-1 group flex flex-col items-center text-center">
                    <div class="w-14 h-14 bg-[#23bfe2]/10 rounded-2xl flex items-center justify-center text-[#23bfe2] mb-6 group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                        <i data-lucide="layers" class="w-7 h-7"></i>
                    </div>
                    <h4 class="text-xl font-bold text-[#0B2035] mb-3">Integrated Treatment</h4>
                    <p class="text-gray-500 font-light leading-relaxed">Medication, Deep TMS, SPRAVATO, IV therapy, and lifestyle medicine all under one roof.</p>
                </div>

                <div class="bg-white rounded-[2rem] p-8 shadow-soft border border-gray-100 hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 reveal reveal-delay-2 group flex flex-col items-center text-center">
                    <div class="w-14 h-14 bg-[#23bfe2]/10 rounded-2xl flex items-center justify-center text-[#23bfe2] mb-6 group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                        <i data-lucide="laptop" class="w-7 h-7"></i>
                    </div>
                    <h4 class="text-xl font-bold text-[#0B2035] mb-3">In-Person & Telehealth</h4>
                    <p class="text-gray-500 font-light leading-relaxed">Flexible appointment options at our Newport Beach and San Juan Capistrano offices or via secure video.</p>
                </div>

                <div class="bg-white rounded-[2rem] p-8 shadow-soft border border-gray-100 hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 reveal reveal-delay-3 group flex flex-col items-center text-center">
                    <div class="w-14 h-14 bg-[#23bfe2]/10 rounded-2xl flex items-center justify-center text-[#23bfe2] mb-6 group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                        <i data-lucide="file-check-2" class="w-7 h-7"></i>
                    </div>
                    <h4 class="text-xl font-bold text-[#0B2035] mb-3">Insurance Accepted</h4>
                    <p class="text-gray-500 font-light leading-relaxed">In-network with most major insurances including Aetna, Anthem, Cigna, United, Medicare, TriCare.</p>
                </div>
            </div>
        </div>
    </section>

    <!-- FAQ Section -->
    <section class="py-16 bg-[#0B2035] text-white relative border-t-[8px] border-[#23bfe2]">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16 reveal">
                <h2 class="text-sm font-bold tracking-widest text-[#23bfe2] uppercase mb-4">Common Questions</h2>
                <h3 class="text-4xl md:text-5xl font-bold">Frequently Asked Questions</h3>
            </div>

            <div class="space-y-4">
                <div class="bg-white/5 border border-white/10 rounded-2xl overflow-hidden reveal">
                    <button class="faq-toggle w-full px-8 py-6 text-left font-bold text-xl flex items-center justify-between focus:outline-none hover:bg-white/5 transition-colors group">
                        <span>What is Deep TMS for PTSD and how does it work?</span>
                        <div class="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-[#23bfe2] group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                            <i data-lucide="plus" class="w-5 h-5 transition-transform duration-300"></i>
                        </div>
                    </button>
                    <div class="px-8 pb-8 text-gray-300 font-light leading-relaxed text-lg hidden faq-content">
                        <div class="pt-2 border-t border-white/10 mt-2">
                            Deep TMS (Transcranial Magnetic Stimulation) for PTSD uses magnetic pulses to stimulate neural pathways involved in trauma, fear processing, and stress regulation. By activating under-functioning brain regions, Deep TMS helps reduce symptoms like intrusive memories, hypervigilance, and avoidance. It's non-invasive and typically done without anesthesia.
                        </div>
                    </div>
                </div>

                <div class="bg-white/5 border border-white/10 rounded-2xl overflow-hidden reveal">
                    <button class="faq-toggle w-full px-8 py-6 text-left font-bold text-xl flex items-center justify-between focus:outline-none hover:bg-white/5 transition-colors group">
                        <span>How will I know if Deep TMS for PTSD is working, and is follow-up required?</span>
                        <div class="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-[#23bfe2] group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                            <i data-lucide="plus" class="w-5 h-5 transition-transform duration-300"></i>
                        </div>
                    </button>
                    <div class="px-8 pb-8 text-gray-300 font-light leading-relaxed text-lg hidden faq-content">
                        <div class="pt-2 border-t border-white/10 mt-2">
                            Progress is monitored via symptom checklists, clinician evaluation, and sometimes patient-reported outcomes. Many patients notice improvements in sleep, reduction in nightmares or flashbacks, and less hypervigilance within 2-4 weeks. Follow-up treatments or maintenance sessions may be suggested if symptoms recur or remain at a lower level.
                        </div>
                    </div>
                </div>

                <div class="bg-white/5 border border-white/10 rounded-2xl overflow-hidden reveal">
                    <button class="faq-toggle w-full px-8 py-6 text-left font-bold text-xl flex items-center justify-between focus:outline-none hover:bg-white/5 transition-colors group">
                        <span>What are the possible side effects for Deep TMS when treating PTSD?</span>
                        <div class="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-[#23bfe2] group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                            <i data-lucide="plus" class="w-5 h-5 transition-transform duration-300"></i>
                        </div>
                    </button>
                    <div class="px-8 pb-8 text-gray-300 font-light leading-relaxed text-lg hidden faq-content">
                        <div class="pt-2 border-t border-white/10 mt-2">
                            Common side effects include mild scalp discomfort or headache during the first few sessions, lightheadedness, or twitching near the site of stimulation. These generally lessen over time. Serious side effects are rare. Our Newport Beach team constantly monitors you and adjusts parameters for your comfort and safety.
                        </div>
                    </div>
                </div>

                <div class="bg-white/5 border border-white/10 rounded-2xl overflow-hidden reveal">
                    <button class="faq-toggle w-full px-8 py-6 text-left font-bold text-xl flex items-center justify-between focus:outline-none hover:bg-white/5 transition-colors group">
                        <span>How long is a course of Deep TMS for PTSD, and what does a typical session look like?</span>
                        <div class="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-[#23bfe2] group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                            <i data-lucide="plus" class="w-5 h-5 transition-transform duration-300"></i>
                        </div>
                    </button>
                    <div class="px-8 pb-8 text-gray-300 font-light leading-relaxed text-lg hidden faq-content">
                        <div class="pt-2 border-t border-white/10 mt-2">
                            Treatment generally involves daily sessions, five days per week, over about 4-6 weeks. Each session lasts around 20-40 minutes depending on protocol. During the session you sit in a chair, an electromagnetic coil is placed on your scalp, and you may feel clicking and tapping sensations. After each session you can leave and resume normal activities.
                        </div>
                    </div>
                </div>

                <div class="bg-white/5 border border-white/10 rounded-2xl overflow-hidden reveal">
                    <button class="faq-toggle w-full px-8 py-6 text-left font-bold text-xl flex items-center justify-between focus:outline-none hover:bg-white/5 transition-colors group">
                        <span>Do you treat veterans with PTSD?</span>
                        <div class="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-[#23bfe2] group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                            <i data-lucide="plus" class="w-5 h-5 transition-transform duration-300"></i>
                        </div>
                    </button>
                    <div class="px-8 pb-8 text-gray-300 font-light leading-relaxed text-lg hidden faq-content">
                        <div class="pt-2 border-t border-white/10 mt-2">
                            Yes. Dr. LaFrenierre holds a Harvard fellowship in Trauma and Recovery. We serve veterans and accept TriCare and TriWest insurance.
                        </div>
                    </div>
                </div>

                <div class="bg-white/5 border border-white/10 rounded-2xl overflow-hidden reveal">
                    <button class="faq-toggle w-full px-8 py-6 text-left font-bold text-xl flex items-center justify-between focus:outline-none hover:bg-white/5 transition-colors group">
                        <span>Can PTSD be treated alongside other conditions?</span>
                        <div class="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-[#23bfe2] group-hover:bg-[#23bfe2] group-hover:text-[#0B2035] transition-colors">
                            <i data-lucide="plus" class="w-5 h-5 transition-transform duration-300"></i>
                        </div>
                    </button>
                    <div class="px-8 pb-8 text-gray-300 font-light leading-relaxed text-lg hidden faq-content">
                        <div class="pt-2 border-t border-white/10 mt-2">
                            Absolutely. PTSD often co-occurs with depression, anxiety, and substance use. Our integrative approach addresses all co-occurring conditions simultaneously.
                        </div>
                    </div>
                </div>
            </div>

            <div class="text-center mt-12">
                <p class="text-gray-400 font-light">Have more questions? <a href="/contact/" class="text-[#23bfe2] font-semibold hover:text-white transition-colors underline underline-offset-4 decoration-[#23bfe2]/30 hover:decoration-white">Contact our clinical team.</a></p>
            </div>
        </div>
    </section>

    <!-- Final CTA -->
    <section class="py-16 relative overflow-hidden bg-white">
        <div class="absolute inset-0 z-0">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/anxiety_relief.png" alt="Path to Peace" class="w-full h-full object-cover opacity-10 filter sepia-[0.3]">
            <div class="absolute inset-0 bg-gradient-to-b from-white via-white/80 to-white"></div>
        </div>

        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10 reveal">
            <h2 class="text-4xl md:text-5xl lg:text-6xl font-bold text-[#0B2035] mb-8 leading-tight">
                Take the First Step Toward <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#23bfe2] to-[#1da1bf]">Healing</span>
            </h2>
            <p class="text-xl text-gray-600 mb-12 font-light max-w-2xl mx-auto leading-relaxed">
                Schedule a consultation with our team and discover a personalized treatment plan designed for your unique needs.
            </p>
            <div class="flex flex-col sm:flex-row justify-center gap-6 items-center">
                <a href="/contact/" class="group relative inline-flex items-center justify-center bg-[#0B2035] hover:bg-[#23bfe2] text-white hover:text-[#0B2035] font-bold px-10 py-5 rounded-full transition-all duration-300 shadow-xl hover:-translate-y-1 hover:shadow-2xl w-full sm:w-auto text-lg">
                    <span class="relative z-10 flex items-center gap-2">
                        Schedule Your Consultation
                        <i data-lucide="arrow-up-right" class="w-6 h-6 transform group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform"></i>
                    </span>
                </a>
                <a href="tel:9499965355" class="text-[#0B2035] font-bold text-lg hover:text-[#23bfe2] transition-colors flex items-center gap-2">
                    <i data-lucide="phone" class="w-5 h-5"></i> (949) 996-5355
                </a>
            </div>
        </div>
    </section>
</main>

<script>
    document.addEventListener('DOMContentLoaded', () => {
        const triggers = document.querySelectorAll('.faq-toggle');
        triggers.forEach(trigger => {
            trigger.addEventListener('click', () => {
                const icon = trigger.querySelector('[data-lucide="plus"]');
                const content = trigger.nextElementSibling;

                if (content.classList.contains('hidden')) {
                    triggers.forEach(other => {
                        if (other !== trigger) {
                            other.nextElementSibling.classList.add('hidden');
                            other.querySelector('[data-lucide="plus"]').style.transform = 'rotate(0deg)';
                        }
                    });

                    content.classList.remove('hidden');
                    if (icon) icon.style.transform = 'rotate(45deg)';
                } else {
                    content.classList.add('hidden');
                    if (icon) icon.style.transform = 'rotate(0deg)';
                }
            });
        });
    });
</script>

    <!-- Also Explore -->
    <section class="py-12 bg-gray-50 border-t border-gray-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <p class="text-xs font-bold tracking-widest text-[#23bfe2] uppercase mb-6">Also Explore</p>
            <div class="flex flex-wrap gap-3">
                <a href="/tms-therapy/" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-[#0B2035]/15 text-[#0B2035] text-sm font-medium hover:bg-[#0B2035] hover:text-white transition-all">Deep TMS Therapy</a>
                <a href="/spravato-therapy/" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-[#0B2035]/15 text-[#0B2035] text-sm font-medium hover:bg-[#0B2035] hover:text-white transition-all">SPRAVATO® Treatment</a>
                <a href="/medication-management/" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-[#0B2035]/15 text-[#0B2035] text-sm font-medium hover:bg-[#0B2035] hover:text-white transition-all">Medication Management</a>
                <a href="/tms-for-depression-orange-county/" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-[#0B2035]/15 text-[#0B2035] text-sm font-medium hover:bg-[#0B2035] hover:text-white transition-all">Depression Treatment</a>
                <a href="/contact/" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-[#0B2035]/15 text-[#0B2035] text-sm font-medium hover:bg-[#0B2035] hover:text-white transition-all">Schedule a Consultation</a>
            </div>
        </div>
    </section>

<?php get_footer(); ?>